...
func (s *TrieStats) PrintStats() {
	fmt.Printf("merkle:\t%d\t%d\t%.3f\t%d\n", s.get, s.put, float64(s.walk)/float64(s.get+s.put), s.decode)
	s.Reset()
}

func (t *Trie) PrintStats() {
	t.stats.PrintStats()
	t.db.PrintStats()
}
