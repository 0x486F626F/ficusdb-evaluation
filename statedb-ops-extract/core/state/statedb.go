...
func NewExtractor(root common.Hash, db Database) (*StateDB, error) {
	fmt.Println("newstatedb", root.Hex())
	tr, err := db.OpenTrie(root)
	if err != nil {
		return nil, err
	}
...
func (s *StateDB) GetBalance(addr common.Address) *uint256.Int {
	...
	res := common.U2560
	if stateObject != nil {
		res = stateObject.Balance()
	}
	if s.extract_ops {
		fmt.Println("getbalance", addr.Hex(), res)
	}
	return res
}
...
func (s *StateDB) GetNonce(addr common.Address) uint64 {
	...
	res := uint64(0)
	if stateObject != nil {
		res = stateObject.Nonce()
	}
	if s.extract_ops {
		fmt.Println("getnonce", addr.Hex(), res)
	}
	return res
}
...
func (s *StateDB) GetCodeHash(addr common.Address) common.Hash {
	...
	res := common.Hash{}
	if stateObject != nil {
		res = common.BytesToHash(stateObject.CodeHash())
	}
	if s.extract_ops {
		fmt.Println("getcodehash", addr.Hex(), res.Hex())
	}
	return res
}
...
func (s *StateDB) GetState(addr common.Address, hash common.Hash) common.Hash {
	...
	res := common.Hash{}
	if stateObject != nil {
		res = stateObject.GetState(hash)
	}
	if s.extract_ops {
		fmt.Println("getstate", addr.Hex(), hash.Hex(), res.Hex())
	}
	return res
}
...
func (s *StateDB) AddBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) uint256.Int {
	if s.extract_ops {
		fmt.Println("addbalance", addr.Hex(), amount)
	}
...
func (s *StateDB) SubBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) uint256.Int {
	if s.extract_ops {
		fmt.Println("subbalance", addr.Hex(), amount)
	}
...
func (s *StateDB) SetBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) {
	if s.extract_ops {
		fmt.Println("setbalance", addr.Hex(), amount)
	}
...
func (s *StateDB) SetNonce(addr common.Address, nonce uint64) {
	if s.extract_ops {
		fmt.Println("setnonce", addr.Hex(), nonce)
	}
...
func (s *StateDB) SetCode(addr common.Address, code []byte) {
	if s.extract_ops {
		fmt.Println("setcode", addr.Hex(), hex.EncodeToString(code))
	}
...
func (s *StateDB) SelfDestruct(addr common.Address) uint256.Int {
	if s.extract_ops {
		fmt.Println("removeaccount", addr.Hex())
	}
...
func (s *StateDB) SelfDestruct6780(addr common.Address) (uint256.Int, bool) {
	if s.extract_ops {
		fmt.Println("removeaccount6780", addr.Hex())
	}
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
...
func (s *StateDB) CreateAccount(addr common.Address) {
	if s.extract_ops {
		fmt.Println("createaccount", addr.Hex())
	}
	s.createObject(addr)
}
...
func (s *StateDB) Snapshot() int {
	sid := s.journal.snapshot()
	if s.extract_ops {
		fmt.Println("snapshot", sid)
	}
	return sid
}
...
func (s *StateDB) RevertToSnapshot(revid int) {
	if s.extract_ops {
		fmt.Println("revertsnapshot", revid)
	}
...
func (s *StateDB) Finalise(deleteEmptyObjects bool) {
	if s.extract_ops {
		fmt.Println("finalise", deleteEmptyObjects)
	}
...
func (s *StateDB) Commit(block uint64, deleteEmptyObjects bool) (common.Hash, error) {
	...
	if s.extract_ops {
		fmt.Println("commit", block, deleteEmptyObjects, ret.root)
	}
	return ret.root, nil
}
